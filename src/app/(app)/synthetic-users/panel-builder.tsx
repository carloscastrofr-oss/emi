...
const panelFormSchema = z.object({
  ...
  segments: z
    .array(segmentSchema)
    .min(1, "Añade al menos un segmento.")
    .max(5, "No más de 5 segmentos."),
});
  ...
  return (
    <Card className="rounded-expressive shadow-e2">
      <CardHeader>
        ...
        <CardDescription>
          Define los atributos de los usuarios que quieres simular. La IA generará perfiles
          realistas basados en tu configuración.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} placeholder="Ej: Test de checkout para CA-β" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormItem>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => handleAddSegment(segmentInput)}
                  disabled={segmentFields.length >= 5 || !segmentInput.trim()}
                >
              ...
              <div className="flex flex-wrap gap-1 pt-2">
                {segmentSuggestions.map((suggestion) => (
                  <Button
                    ...
                    disabled={
                      segmentFields.length >= 5 || segmentFields.some((f) => f.value === suggestion)
                    }
                  >
                    {suggestion}
                  </Button>
