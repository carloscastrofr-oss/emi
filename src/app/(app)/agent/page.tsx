...
const agents = [
  {
    ...
    description:
      "Ejecuta una auditoría de accesibilidad en una URL para identificar problemas de WCAG.",
    icon: Accessibility,
  ...
  },
  {
    ...
    description:
      "Analiza el texto de la UI y los comentarios para proponer mejoras en el micro-copy.",
    icon: MessageSquare,
    formFields: [
      { name: "uiText", label: "Texto de la UI" },
      { name: "userFeedback", label: "Feedback del Usuario (Opcional)" },
    ],
  ...
  },
  {
    ...
    description:
      "Evalúa la deuda de diseño, identifica componentes divergentes y estima el ROI perdido.",
    icon: Trash2,
            ...
            <TableBody>
              {isLoading ? (
                ...
                recommendations.map((rec) => (
                  <TableRow key={rec.id}>
                    <TableCell>
                      <Badge variant={getBadgeVariant(rec.agent)}>
                        {agentNameTranslations[rec.agent]}
                      </Badge>
                    </TableCell>
