...
import {
  ...
  GenerateDesignStrategyOutput,
} from "@/ai/flows/generate-design-strategy";
...
const formSchema = z.object({
  vision: z
    .string()
    .min(1, "La visión es requerida.")
    .max(140, "La visión no debe exceder los 140 caracteres."),
  valueProp: z
    .string()
    .min(1, "La propuesta de valor es requerida.")
    .max(200, "La propuesta de valor no debe exceder los 200 caracteres."),
  okrs: z.array(okrSchema).min(1, "Debe haber al menos un OKR.").max(3, "No más de 3 OKRs."),
  personas: z
    .array(z.object({ value: z.string().min(1, "Este campo es requerido.") }))
    .min(1, "Añade al menos una persona.")
    .max(5, "No más de 5 personas."),
  principles: z
    .array(z.object({ value: z.string().min(1, "Este campo es requerido.") }))
    .min(1, "Añade al menos un principio.")
    .max(5, "No más de 5 principios."),
...
const ChipInput = ({ control, name, label, placeholder, limit }: ChipInputProps) => {
  const { fields, append, remove } = useFieldArray<FormData>({ control, name });
...
export default function StrategyPage() {
  ...
  async function onSubmit(values: FormData) {
    ...
    const transformChipArrays = (arr: { value: string }[] | undefined) =>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <Card className="rounded-expressive shadow-e2">
            ...
            <CardContent className="space-y-6">
              ...
              {okrFields.map((field, index) => (
                <div
                  key={field.id}
                  className="p-4 border rounded-lg space-y-2 relative bg-background/50"
                >
                  <FormField
                    control={form.control}
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input {...field} placeholder="Ej: Mejorar accesibilidad" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea {...field} placeholder="Ej: A11y-Score ≥ 95" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
              ...
              {okrFields.length < 3 && (
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => appendOkr({ objective: "", krs: "" })}
                >
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <ChipInput
              ...
              placeholder="Ej: Usuarios con baja visión"
              limit={5}
            />
            <ChipInput
              control={form.control}
            ...
            <ChipInput
              control={form.control}
              name="scopeProducts"
              label="Productos en Alcance"
              placeholder="Ej: Producto Principal"
              limit={5}
            />
            <FormField
              control={form.control}
              name="legacyConstraints"
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Restricciones Técnicas o Legacy</FormLabel>
                  <FormControl>
                    <Textarea
                      {...field}
                      placeholder="Describe cualquier dependencia, deuda técnica o restricción importante."
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <Card className="rounded-expressive">
              ...
              <CardContent className="space-y-4">
                <FormField
                  control={form.control}
                  name="governance.accountableRole"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Rol Responsable</FormLabel>
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="Comité de Diseño">Comité de Diseño</SelectItem>
                        </SelectContent>
                      </Select>
                      <FormMessage />
                    </FormItem>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        ...
                        <SelectContent>
                          <SelectItem value="Kanban Simplificado">Kanban Simplificado</SelectItem>
                          <SelectItem value="Git-flow">Git-flow</SelectItem>
                          <SelectItem value="Reunión Semanal">Reunión Semanal</SelectItem>
                        </SelectContent>
                      </Select>
                      <FormMessage />
                    </FormItem>
            ...
            <Card className="rounded-expressive">
              ...
              <CardContent className="space-y-4">
                <FormField
                  control={form.control}
                  name="kpiWeights.csat"
                  render={({ field: { onChange, value } }) => (
                    <FormItem>
                      <FormLabel>Satisfacción del Cliente (CSAT) - Peso: {value}</FormLabel>
                      <FormControl>
                        <Slider
                          value={[value]}
                ...
                <FormField
                  control={form.control}
                  name="kpiWeights.a11y"
                  render={({ field: { onChange, value } }) => (
                    <FormItem>
                      <FormLabel>Accesibilidad (A11y) - Peso: {value}</FormLabel>
                      <FormControl>
                        <Slider
                          value={[value]}
                          onValueChange={(v) => onChange(v[0])}
                ...
                <FormField
                  control={form.control}
                  name="kpiWeights.adoption"
                  render={({ field: { onChange, value } }) => (
                    <FormItem>
                      <FormLabel>Adopción de Componentes - Peso: {value}</FormLabel>
                      <FormControl>
                        <Slider
                          value={[value]}
                          onValueChange={(v) => onChange(v[0])}
                  ...
                  <CardContent className="space-y-4 text-sm">
                    <div>
                      ...
                      <p className="text-muted-foreground">
                        {generatedResult.designInterpretation.strategicSummary}
                      </p>
                    </div>
                    <div>
                      <h4 className="font-semibold mb-1">Objetivos de UX</h4>
                      <p className="text-muted-foreground">
                        {generatedResult.designInterpretation.userExperienceGoals}
                      </p>
                    </div>
                    <div>
                      <h4 className="font-semibold mb-1">Gobernanza Práctica</h4>
                      <p className="text-muted-foreground">
                        {generatedResult.designInterpretation.governanceModel}
                      </p>
                    </div>
                    <div>
                      <h4 className="font-semibold mb-1">Sugerencias de Componentes</h4>
                      <ul className="list-disc list-inside text-muted-foreground space-y-1">
                        {generatedResult.designInterpretation.componentSuggestions.map(
                          (suggestion, index) => (
              ...
              <TabsContent value="markdown">
                <Card className="rounded-expressive">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Este es el resultado de tu brief. Puedes copiar el Markdown.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <Alert>
