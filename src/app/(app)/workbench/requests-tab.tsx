...
function NewRequestDialog({ open, onOpenChange }: { open: boolean, onOpenChange: (open: boolean) => void }) {
    ...
    async function onSubmit(values: z.infer<typeof newRequestSchema>) {
      if (!userProfile) {
        toast({ title: "Error", description: "Debes iniciar sesión para crear una solicitud.", variant: "destructive" });
        return;
      }
      setIsLoading(true);
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4 py-4">
              <FormField control={form.control} name="title" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="Ej: Componente DatePicker" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="description" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Textarea placeholder="Describe el caso de uso, requerimientos y comportamiento esperado." {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="priority" render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    ...
                    <SelectContent>
                      {requestPriorities.map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}
                    </SelectContent>
                  </Select>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="figmaFileUrl" render={({ field }) => (
                <FormItem>
                  <FormLabel>Link a Figma (Opcional)</FormLabel>
                  <FormControl><Input placeholder="https://figma.com/..." {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <DialogFooter>
...
export function RequestsTab() {
  ...
  useEffect(() => {
        ...
        setRequests([
            {
                ...
                history:[{status:'Solicitado',byUid:'designer123',at:Timestamp.now()},
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
        const newRequests = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as ComponentRequest));
  ...
  const getPriorityVariant = (priority: RequestPriority): "destructive" | "secondary" | "outline" => {
    switch (priority) {
      ...
      <CardHeader>
        <div className="flex justify-between items-start">
            <div>
                ...
                <CardDescription>Pipeline de nuevas propuestas de componentes y mejoras.</CardDescription>
            </div>
            ...
            {isLoading ? (
              ...
              requests.map((req) => (
                ...
                >
                  ...
                  <TableCell>
                    <Badge variant={getStatusVariant(req.status)} className={req.status === 'Listo' ? 'bg-green-600 text-white' : ''}>
                      {req.status}
                    </Badge>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2 text-xs">
                        <Avatar className="h-6 w-6">
                            <AvatarImage src={`https://placehold.co/40x40.png`} alt={req.requesterUid} data-ai-hint="person avatar" />
                  ...
                  <TableCell>
                    {req.championUid ? (
                         <div className="flex items-center gap-2 text-xs">
                            <Avatar className="h-6 w-6">
                                <AvatarImage src={`https://placehold.co/40x40.png`} alt={req.championUid} data-ai-hint="person avatar" />
