...
export function OnboardingTour({ run, setRun }: OnboardingTourProps) {
  ...
  const tourSteps = React.useMemo(() => {
    ...
    const tourConfig = ONBOARDING_TOURS.find(t => t.role === role) || ONBOARDING_TOURS[ONBOARDING_TOURS.length -1];
    return tourConfig.steps.map(s => ({ ...s, disableBeacon: true }));
  }, [userProfile?.role]);

  const handleJoyrideCallback = async (data: CallBackProps) => {
    const { event, step, type } = data;
    ...
    if (event === 'tour:end' || event === 'tour:status') {
        setRun(false);
    }
  };
