...
export function AssignRiskModal({ risk, open, onClose, onAssign }: AssignRiskModalProps) {
    ...
    const handleSave = async () => {
        if (!selectedAssignee || !userProfile) {
            toast({ title: "Error", description: "Selecciona un responsable e inicia sesión.", variant: "destructive" });
            return;
        }
        ...
        if (!isFirebaseConfigValid) {
            ...
            const result = await assignRisk(risk.id, assigneeUser.uid, assigneeUser.name, userProfile.uid);
            if (!result.success) {
        ...
        setIsLoading(false);
        toast({ title: "Riesgo Asignado", description: `El riesgo "${risk.title}" ha sido asignado a ${assigneeUser.name}.`});
        onClose();
    };
